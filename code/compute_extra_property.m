function data = compute_extra_property (data, Prefs)
    ...
    %   num_dim, trace_idx, num_loc_per_trace, attr_names ?origin, xyz, xyz to xnm, ynm, znm?
    %
    %data.cali = calibration: add
    %   RIMF, loc_precision, local_density, pixel_size
    %
    %data.channel = channel: add
    %   dcr (last or all iter), cut1, cut2, keep_ch3, do_trace, ?RGB
    %
    ...
    % xyz nm with re-zeroed origin moved all to render localization function
