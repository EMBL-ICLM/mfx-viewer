function load_data (app, file_name, folder)    
    ...
    if ~valid
        ...
        app.StatusTextArea.Value = ["Data loaded:"; data.file.name];
    end
    update_Prefs_with_file (app, data.file.folder, data.file.name);


    %% 3, compute and store additional data properties: prop, attr, ...
    data = compute_extra_property (data, app.Prefs);


    %% 4, make the freshly loaded data as the active data and append to MINFLUX Viewer app
    set_active_data (app, data);
    

    %% 5, check additional action in prefs, associated with loading data
    data_actions = app.Prefs.data;
    
    % if now app stores more than 1 dataset, show Data Selector app anyway
    if data_actions.show_data_info || ~first_load
